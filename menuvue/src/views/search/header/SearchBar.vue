    ...
    <header>
        <div class="search-bar-container">
            ...
            <div class="search-bar-input-container">
                <input type="text" v-model="keyword" @keyup.enter="search()" placeholder="검색어를 입력하세요">
                <button @click="search()">검색</button>
            </div>
            <div class="hot-key-container">
                <div @mouseleave="hotVisible = false" @mouseover="hotVisible = true">
                    <span>인기 검색어: </span>
                    <span class="hot-keyword-container" @click=" keyword = hotKeyword[hotKeyIdx];">{{
                        hotKeyword[hotKeyIdx] }}</span>
                    <ul v-if="hotVisible">
                        <li v-for="hotKey in hotKeyword" :key=hotKey>
                            <a href="#" @click="keyword = hotKey;">{{
                                hotKey }}</a>
                        </li>
                    </ul>
                </div>
